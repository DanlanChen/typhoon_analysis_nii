	...
	for key in train_folder:
		...
		if train_x.shape[0] >0:
			...
			if len(trainPredict) == len(trainPredict_2[20:]):
				...
				if int(len(trainPredict) *0.1) >=1:
					count +=1 
					train_error_10 += math.sqrt(mean_squared_error(trainPredict[:int(0.1 * len(trainPredict))] ,trainY[:int(0.1*len(trainPredict))]))
					train_error_10_2 += math.sqrt(mean_squared_error(trainPredict_2[20:0.1*len(trainPredict) +20],trainY[:int(0.1*len(trainPredict))]))
					train_error_10_p += np.sum(np.power((trainPredict[:int(0.1 * len(trainPredict))] -trainY[:int(0.1*len(trainPredict))]),2))/np.sum(np.power((trainPredict-trainY),2))
					train_error_10_2_p += np.sum(np.power((trainPredict_2[20:0.1*len(trainPredict) +20]-trainY[:int(0.1*len(trainPredict))]),2))/np.sum(np.power((trainPredict_2[20:]-trainY),2))
					if 6 in train_type:
	...
	for key in test_folder:
		...
		# # # invert predictions
			...
			if len(testPredict) == len(testPredict_2[20:]):
				...
				print 'writing to csv' + key
				if int(len(testPredict) *0.1) >=1:
					count += 1
					test_error_10 += math.sqrt(mean_squared_error(testPredict[:int(0.1 * len(testPredict))] ,testY[:int(0.1*len(testPredict))]))
					test_error_10_2 += math.sqrt(mean_squared_error(testPredict_2[20:0.1*len(testPredict) +20],testY[:int(0.1*len(testPredict))]))
					test_error_10_p += np.sum(np.power((testPredict[:int(0.1 * len(testPredict))] -testY[:int(0.1*len(testPredict))]),2))/np.sum(np.power((testPredict-testY),2))
					test_error_10_2_p += np.sum(np.power((testPredict_2[20:0.1*len(testPredict) +20]-testY[:int(0.1*len(testPredict))]),2))/np.sum(np.power((testPredict_2[20:]-testY),2))
					if 6 in test_type:
