...
# 		print (file_path not exists)
	...
	dataset_image_path = 'test_file/dataset_imageset_crop.hdf5'
	...
	for key in train_folder:
		...
		# dataset_image = prepare_dataset.dataset_2(image_folder)
		# dataset_input = get_fc2(vgg_model,dataset_image)
		# dataset_image = np.array(dataset_input)
		dataset_image = np.array(hf_image.get(key))
		dataset_intensity = np.array(hf_intensity.get(key))
		# print (dataset_image.shape,'dataset_image')
		train_x,train_y = prepare_dataset.extend_dataset_2_zero(dataset_image, dataset_intensity,look_back = look_back)
		train_x = np.array(train_x,dtype = 'float32')
		train_y = np.array(train_y,dtype = 'float32')
		if train_x.shape[0] >0:
			train_predict_image = 'test_file/prediction_output_6_zero_crop/' + str(key)+'_'+str(look_back)+'crop_train.png' 
			...
			plt.plot(list(trainPredict[20:20000,0]),'r--',label= 'train_predict')
	...
	for key in test_folder:
		...
		dataset_image = np.array(hf_image.get(key))
		dataset_intensity = np.array(hf_intensity.get(key))
		test_x,test_y = prepare_dataset.extend_dataset_2_zero(dataset_image, dataset_intensity,look_back = look_back)
		...
		# # # invert predictions
			...
			test_predict_image = 'test_file/prediction_output_6_zero_crop/' + str(key)+'_'+str(look_back)+'crop_test.png' 
			fig = plt.figure()
			plt.title('test_predicts_look_back ' + str(look_back) +', typhoon number ' + str(key))
			plt.plot(list(testPredict[20:10000,0]),'r--',label= 'test_predict')
