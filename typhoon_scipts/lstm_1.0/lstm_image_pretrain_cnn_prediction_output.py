	...
	for key in train_folder:
		...
		if len(dataset_intensity) > look_back:
			...
			if train_x.shape[0] >0:
				...
				plt.plot(list(trainY[:20000]), 'g--',label = 'train_true')
	...
	for key in test_folder:
		...
		if len(dataset_intensity) > look_back:
			...
			# # # invert predictions
				...
				test_predict_image = 'test_file/prediction_output_6_04_13_old_version/' + str(key)+'_'+str(look_back)+'_test.png' 
				fig = plt.figure()
				...
				plt.plot(list(testY[:10000]), 'g--',label = 'test_true')
				plt.xlabel('typhoon_image')
				plt.ylabel('typhoon intensity')
				plt.ylim([850,1050])
				plt.legend(loc = 'upper left', shadow =True)
