...
def pretrain_model(look_back,batch_size):
	...
	# model.add(Flatten())
	model.add(Dense(8,batch_input_shape = (batch_size,4096)))
...
def main():
	...
	ModelCheckpoint_file = 'test_file/classify_vgg_16_inidividual.hdf5'
	look_back = config.look_back
	img_rows,img_cols = config.img_rows,config.img_cols
	subdir_list = []
	# hist_path = config.hist_path
	hist_path = 'test_file/classify_vgg_16_inidividual_hist.json'
...
# 		print (file_path not exists)
	...
	for key in train_folder:
		...
		train_x += list(dataset_image)
		train_y += list(dataset_type)
	...
	for key in test_folder:
		...
		dataset_type = np.array(hf_type.get(key))
		test_x += list(dataset_image)
		test_y+=list(dataset_type)
		# if len(dataset_image) > look_back:
		# 	data_x,data_y = prepare_dataset.extend_dataset_2(dataset_image, dataset_type,look_back = look_back)
		# 	test_x += data_x
		# 	test_y += data_y
	...
	print(model.summary())
	y_train = np_utils.to_categorical(train_y, 8)
	y_test = np_utils.to_categorical(test_y, 8)
	print y_train.shape
	print y_test.shape
	...
	for i in range(10000):
		...
		# print('start train')
		hist = model.fit(train_x, y_train, nb_epoch=1, batch_size=64, verbose=2, validation_split=0.1,shuffle=False)
		print hist.history
		# model.reset_states()
	...
	train_confusion_matrix_path = 'test_file/confusion_matrix_train_extend_normalize_individual'+ str( look_back) +'.json'
	with open(train_confusion_matrix_path, 'w') as f:
		json.dump(train_cm,f)

	test_cm = test_cm.tolist()
	test_confusion_matrix_path = 'test_file/confusion_matrix_test_extend_normalize_individual'+ str(look_back) +'.json'
