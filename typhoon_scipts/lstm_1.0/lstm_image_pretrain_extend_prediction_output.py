...
def pretrain_model(look_back,batch_size):
	"""
	lstm model which used pretrained cnn-vgg 16 last fully connected layer output 4096*1
	this lstm is a one layer lstm with neuron of 300
	"""
	model =Sequential()
...
# 		print (file_path not exists)
	...
	for key in train_folder:
		...
		if len(dataset_intensity) > look_back:
			train_x,train_y = prepare_dataset.extend_dataset_2(dataset_image, dataset_intensity,look_back = look_back)
			
			train_x = np.array(train_x,dtype = 'float32')
			train_y = np.array(train_y,dtype = 'float32')
			if train_x.shape[0] >0:
				train_predict_image = 'test_file/prediction_output_24/' + str(key)+'_'+str(look_back)+'_train.png' 
	...
	for key in test_folder:
		...
		dataset_intensity = np.array(hf_intensity.get(key))
		if len(dataset_intensity) > look_back:
			test_x,test_y = prepare_dataset.extend_dataset_2(dataset_image, dataset_intensity,look_back = look_back)
			...
			# # # invert predictions
				...
				test_predict_image = 'test_file/prediction_output_24/' + str(key)+'_'+str(look_back)+'_test.png' 
				fig = plt.figure()
