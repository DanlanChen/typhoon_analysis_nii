...
def main():
	...
	print('ModelCheckpoint_file',ModelCheckpoint_file)#config.ModelCheckpoint_file
	look_back = 1
	batch_size = 1
	...
	for i in np.arange(0,len(train_file_list_copy),12):#len(train_file_list_copy)
		...
		for i in range(100):
			hist = model.fit(trainX, trainY, nb_epoch=1, batch_size=batch_size, verbose=2, validation_split=0.1,shuffle=False)
			model.reset_states()
